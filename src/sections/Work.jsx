...
const Work = () => {
    ...
    () => {
      ...
      gsap.to(projectsRef.current, {
        ...
        scrollTrigger: {
          ...
          start: "center center",
          end: () => `+=${projectsWidth * 1.5}`, // 1.5x çarpan ekledik - son proje tam görünsün
          pin: true,
          scrub: 1,
  ...
  return (
    <>
      <div
        ref={workRef}
        className="min-h-screen bg-white text-black py-24 lg:py-40 overflow-hidden rounded-tl-[60px] rounded-tr-[60px] relative z-10 -mt-1"
      >
        {/* Title Wrapper */}
        ...
        <div ref={projectsRef}>
          ...
          <div className="flex gap-4 lg:gap-8 ms-4 lg:ms-[40%] mt-6">
            {projects.map(({ id, name, image, link, isPortrait }) => (
              <a
                key={id}
                href={link}
                className={`relative rounded-2xl w-full block overflow-hidden group ${
                  isPortrait
                    ? "min-w-[240px] lg:min-w-[300px] h-[480px] lg:h-[600px]" // Dikey format (9:16)
                    : "min-w-[340px] lg:min-w-xl h-72 lg:h-96" // Yatay format (16:9)
                }`}
              >
                {/* Project Image */}
                <img
